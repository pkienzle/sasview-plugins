...
//std::printf("loop %d %g %ld\n",loop,weight,_model.looporder.size());
  double simple=0.;
  const PolydisperseParameter *poly=NULL;
  int p = _model.looporder[loop];
  int n;
  bool is_polydisperse = _model.pars[p].Flags&PF_Polydisperse;
  if (is_polydisperse) {
	const PolydisperseParameter& ref = _pars[p];
	poly = &ref;
	n = ref.npoints;
  } else {
	simple = _pars[p];
  ...
  for (int i=0; i < n; i++) {
    double pweight, pvalue;
    if (is_polydisperse) {
	pweight = poly->weights[i];
	pvalue = poly->values[i];
	//if (n>1) std::printf("%d %d %g %g\n",p,i,pvalue,pweight);
    } else {
	pweight = 1.;
	pvalue = simple;
    }
